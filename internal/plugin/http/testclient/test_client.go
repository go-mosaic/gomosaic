// Package testclient предоставляет генератор тестов для клиентов.
// Он позволяет генерировать тесты, которые проверяют корректность работы клиентов, используя фейкер-данные для заполнения полей структур.
package testclient
