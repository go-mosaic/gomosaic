// Package flatten предоставляет утилиты для преобразования вложенной структуры в одномерную.
package flatten
	...
	if hasChildren {
		...
		if v.Type.IsSlice {
			...
			for _, v := range p.varsByType(v.Type.ElemType) {
				children = append(children, (&FlattenProcessor{}).Flatten(v)...)
			}
		}
...
// Flatten преобразует переменную в список путей.
