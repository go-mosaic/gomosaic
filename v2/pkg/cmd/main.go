...
//	    )
//
//	    // Можно добавить свои трансформеры
//	    // builder.WithTransformer(myCustomTransformer)
//
//	    // Запускаем CLI
//	    cmd.Run(builder)
//	}
...
// Run запускает CLI с заданным Builder'ом.
func Run(builder *gomosaic.Builder) {
	log.SetFlags(0)

	cmd := &cobra.Command{Use: "gomosaic"}

	cmd.AddCommand(CodegenCmd(builder))

	cobra.CheckErr(cmd.Execute())
}

func CodegenCmd(builder *gomosaic.Builder) *cobra.Command {
	var (
		modfile string
		cmd     = &cobra.Command{
			Use:   "codegen [flags] name packages outputDir",
			Short: "Генерация кода на основе Go-интерфейсов",
			Example: examples(
				"gomosaic codegen http-server-chi ./internal/server",
				"",
				"Параметры:",
				"  name:       Имя плагина для генерации кода.",
